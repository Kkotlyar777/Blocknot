...
export const Folder = () => {
	const { arrAll, currentFile, FolderID } = useAppSelector(
		(state) => state.ArrSlice
	);
	const { addArrEl, DelCurrentFile } = MyFilesSlice.actions;
		...
		>
			<div className={styles.upCont}>
				<span className={styles.FilesPath}>
					<Link
						href={`/`}
						style={{ textDecoration: 'none', color: '#838D94' }}
					>
						Главная
					</Link>{' '}
					{'>'} {FolderID} {'>'} {currentFile}
				</span>
